...
describe('PluginOverlayManager', () => {
  ...
  it('mirrors a preexisting remote OpenCode config dir before adding Orca Lab plugin', () => {
  ...
  it('installs Orca Lab status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
...
describe('resolvePiSourceAgentDir', () => {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not Orca Lab overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
