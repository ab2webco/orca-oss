...
describe('AddRepoHostSelector', () => {
  it('shows a remote host setup menu when Local Mac is the only host', () => {
    ...
    expect(html).toContain('Pair with Orca Lab running on another computer.')
  })
  ...
  it('shows exact update guidance for incompatible runtime hosts', () => {
    ...
    expect(html).toContain('The selected Orca Lab server is too old for this client.')
    expect(html).toContain('Update Orca Lab on the server.')
    expect(html).toContain('aria-disabled="true"')
  })
})
