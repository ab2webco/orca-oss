  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Orca Lab. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Orca Lab. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Orca Lab workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Orca Lab. The project folder on disk will not be deleted.'
