...
vi.mock('./TaskSourceSimpleSetup', () => ({
  CodeHostSetupSteps: (props: {
    ...
    <div data-testid={`code-host-${props.providerLabel}`}>
      {props.unavailable ? (
        <>
          <span>Orca Lab couldn&apos;t check this connection</span>
...
describe('TasksPane', () => {
  ...
  it('shows a retry action instead of setup instructions when preflight is unavailable', async () => {
    ...
    expect(container?.textContent).toContain("Orca Lab couldn't check this connection")
...
describe('TasksPane launch prompt templates', () => {
  beforeEach(() => {
    mocks.readiness = {
      github: { connected: true, checking: false, visible: true },
      gitlab: { connected: true, checking: false, visible: true },
      ...
      jira: { connected: false, checking: false, visible: false },
      plane: { connected: false, checking: false, visible: true }
    }
  })
  ...
  it('persists an edited Linear template on blur', () => {
    const updateSettings = vi.fn()
    render(
      <TasksPane settings={getDefaultSettings('/home/test')} updateSettings={updateSettings} />
    )

    const textarea = screen.getByRole('textbox', { name: /linear launch prompt template/i })
  ...
  it('persists an edited Plane template on blur', () => {
    const updateSettings = vi.fn()
    render(
      <TasksPane settings={getDefaultSettings('/home/test')} updateSettings={updateSettings} />
    )

    const textarea = screen.getByRole('textbox', { name: /plane launch prompt template/i })
    fireEvent.change(textarea, { target: { value: 'Do {{identifier}}' } })
    fireEvent.blur(textarea)
  ...
  it('persists the strip-AI-attribution toggle', () => {
    const updateSettings = vi.fn()
    render(
      <TasksPane settings={getDefaultSettings('/home/test')} updateSettings={updateSettings} />
    )

    fireEvent.click(screen.getByRole('checkbox', { name: /strip ai attribution before writing/i }))

    expect(updateSettings).toHaveBeenCalledWith({ stripAiAttributionFromTickets: false })
  })

  // Why: PROVIDER_META is an exhaustive Record<TaskProvider, …> and this fork's union
  // carries 'plane', so a card must render for it — not just typecheck.
  it('renders a Plane provider card in the setup hub', () => {
    render(<TasksPane settings={getDefaultSettings('/home/test')} updateSettings={vi.fn()} />)

    expect(screen.getByRole('button', { name: /Plane setup steps/i })).toBeTruthy()
  })
})
