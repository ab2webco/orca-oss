...
const KEEP_LOCAL_MAIN_UP_TO_DATE_DESCRIPTION =
  'When you create a workspace, Orca Lab refreshes the remote base and safely fast-forwards your matching local branch, such as main or master. This keeps commands like git diff main...HEAD from comparing against stale history. Orca Lab skips the update if that branch has uncommitted changes or local-only commits.'
...
export function GitPane({
  ...
  const visibleSections = [
    ...
    matchesSettingsSearch(searchQuery, {
      title: translate('auto.components.settings.GitPane.e02ea23a32', 'Orca Lab Attribution'),
      description: translate(
        'auto.components.settings.GitPane.d2eede4c54',
        'Add Orca Lab attribution to commits, PRs, and issues.'
      ),
      keywords: [
      ...
      >
        <div className="space-y-0.5">
          <Label>
            {translate('auto.components.settings.GitPane.e02ea23a32', 'Orca Lab Attribution')}
          </Label>
          <p className="text-xs text-muted-foreground">
            {translate(
              'auto.components.settings.GitPane.d2eede4c54',
