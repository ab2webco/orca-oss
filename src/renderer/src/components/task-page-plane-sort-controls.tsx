...
import type { PlaneWorkItemSortColumn, PlaneWorkItemSortDirection } from './plane-work-item-sorter'

type PlaneSortColumn = {
  id: PlaneWorkItemSortColumn
  label: string
  className?: string
}

type TaskPagePlaneSortControlsProps = {
  direction: PlaneWorkItemSortDirection
  onSort: (column: PlaneWorkItemSortColumn) => void
  orderBy: PlaneWorkItemSortColumn
}

function getPlaneSortColumns(): PlaneSortColumn[] {
  return [
    { id: 'identifier', label: translate('auto.components.TaskPage.planeSortId', 'ID') },
    {
      id: 'title',
      label: translate('auto.components.TaskPage.planeSortWorkItem', 'Work item')
    },
...
export function TaskPagePlaneSortControls({
  direction,
  onSort,
  orderBy
}: TaskPagePlaneSortControlsProps): React.JSX.Element {
  const columns = getPlaneSortColumns()
  const directionLabel =
    direction === 'asc'
      ? translate('auto.components.TaskPage.planeSortAscending', 'ascending')
      : translate('auto.components.TaskPage.planeSortDescending', 'descending')
  const nextDirectionLabel =
    direction === 'asc'
      ? translate('auto.components.TaskPage.planeSortDescending', 'descending')
      : translate('auto.components.TaskPage.planeSortAscending', 'ascending')
  const sortByLabel = translate('auto.components.TaskPage.planeSortBy', 'Sort by')
  const toggleDirectionLabel = translate(
    'auto.components.TaskPage.planeToggleSortDirection',
    'Sort {{value0}}',
  ...
  return (
    <>
      ...
      <div
        data-testid="plane-mobile-sort-controls"
      ...
      >
        ...
        <Select value={orderBy} onValueChange={(value) => onSort(value as PlaneWorkItemSortColumn)}>
          <SelectTrigger
            size="sm"
