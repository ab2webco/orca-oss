...
export default function AiVaultSessionDropLayer({
  ...
  const handleSessionDrop = useCallback(
    (
      ...
      // Why the shared predicate: it covers both the legacy shared home and the
      // per-account home the host now repins, so this gate cannot drift from it.
      ...
      const preparation: Promise<AiVaultPrepareSessionResumeResult> =
        (shouldResolveClaudeUniverse || shouldPrepareCodexHome) &&
      ...
      void preparation
        .then((result) => {
          ...
          if (!startup) {
            ...
            throw new Error(
              result.substituteCodexHome
                ? 'This session was dragged from an older Orca Lab window, so Orca Lab cannot retarget it to the selected Codex account. Resume it from the Session History panel instead.'
                : 'Orca Lab could not prepare this legacy Codex session. Retry resume.'
            )
          }
