...
const ORCA_FEATURE_REQUEST_URL = 'https://github.com/ab2webco/orca-oss/issues/new'
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
        ...
        const unsupportedMessage = `Orca Lab doesn't support ${layoutLabel} project views yet.`
        return (
...
function ErrorState({
  ...
  const copy =
    error.type === 'too_large'
      ? `This view has ${totalCount ?? 'many'} items — too large to render in Orca Lab. Narrow the view's filter on GitHub.`
