...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  // project→repo link) must win over a task-source projectId so the composer opens
  // on that repo. Upstream resolves projectId first, so suppress the seed's project
  // fields for that case — resolving the whole target (not just the repo id) keeps
  // the repo and its host setup consistent. Callers that omit initialRepoId — or
  // pass the item's own repo, as GitHub/GitLab do — resolve exactly as before.
  const prefersInitialRepo = Boolean(
  ...
  const resolvedInitialWorkspaceTarget = resolveWorkspaceCreationTarget({
    ...
    projectId: prefersInitialRepo ? undefined : initialRunSeed.projectId,
    hostId: prefersInitialRepo ? undefined : initialRunSeed.hostId,
    projectHostSetupId: prefersInitialRepo ? undefined : initialRunSeed.projectHostSetupId,
  ...
  // Why: the no-prompt+linked-item path rehydrates the issueCommand template into the prompt.
  // Only github/gitlab qualify — sentinel-based Jira/Linear items would break legacy {{issue}}
  // templates, and Linear/Plane starts have their own launch template, so they must not wait.
  ...
  // Why: Jira/Linear use sentinel numbers that are invalid in legacy {{issue}} templates, and
  // Linear/Plane starts may carry only a neutral issue ref — repo issue-command templates are
  // product-authored workflow direction, so the allowlist keeps them to github/gitlab.
