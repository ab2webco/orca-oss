      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach Orca Lab terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running Orca Lab terminals are hosted by a daemon started by a previous Orca Lab installation. macOS may not apply Orca Lab’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Orca Lab terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
