...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Orca Lab pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Orca Lab session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Orca Lab status hints:', ...statusHints] : []),
    '',
