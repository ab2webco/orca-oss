...
describe('remote terminal stale stream frames', () => {
  ...
  it('keeps a live stream sending after a sibling is retired', async () => {
    const live = await subscribeStream('terminal-1')
    const parked = await subscribeStream('terminal-2')

    parked.close()
    sent = []

    expect(live.sendInput('delivered\r')).toBe(true)
    expect(live.resize(80, 24)).toBe(true)

    expect(inputTextOnWire()).toBe('delivered\r')
    expect(opcodeCount(TerminalStreamOpcode.Resize)).toBe(1)
  })
})
