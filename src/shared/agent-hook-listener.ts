    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Orca Lab install.'
      )
    }
  }
}
...
export function normalizeHookPayload(
  ...
  // Why: Pi/prime-agent session_start and Claude SessionStart all carry resume
  // identity while the TUI is idle; providerSessionOnly makes receivers keep the
  // identity and discard the placeholder row, so none fabricates a turn nobody
  // started.
