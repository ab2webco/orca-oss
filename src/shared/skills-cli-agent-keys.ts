...
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  ...
  // Why: retired (642526200) — z.ai/GLM runs through the regular `claude` CLI, so it
  // reads Claude-owned roots. Kept only so old sessions still resume, and mapped for
  // the same reason as openclaude below. toSkillsCliAgentKeys dedupes via a Set.
  'claude-zai': 'claude-code',
