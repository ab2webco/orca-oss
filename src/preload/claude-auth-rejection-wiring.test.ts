...
describe('Claude auth rejection preload wiring', () => {
  ...
  it('forwards the managed account id to the rejection channel', async () => {
    await import('./index')
    const api = exposeInMainWorld.mock.calls.find(([name]) => name === 'api')?.[1] as PreloadApi
    invoke.mockResolvedValueOnce({})
    invoke.mockClear()

    await api.rateLimits.recordClaudeCredentialRejection('acct_live')

    expect(invoke.mock.calls).toEqual([['rateLimits:recordClaudeCredentialRejection', 'acct_live']])
  })
})
