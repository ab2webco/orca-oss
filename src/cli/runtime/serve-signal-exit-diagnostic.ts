...
export function serveSignalExitError(
  ...
  if (platform !== 'darwin' || signal !== 'SIGABRT') {
    return new RuntimeClientError('runtime_serve_failed', `Orca Lab serve exited via ${signal}.`)
  }
  ...
  return new RuntimeClientError(
    'runtime_serve_failed',
    'Orca Lab serve aborted with SIGABRT on macOS. This most often happens at application startup, when the process cannot register with the macOS window server, which is common in restricted or sandboxed environments, SSH sessions without a GUI login, and CI.',
    {
      nextSteps: [
