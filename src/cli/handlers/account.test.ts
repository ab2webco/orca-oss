...
describe('account CLI handlers', () => {
  ...
  it('marks an account selected for WSL as active in human output', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      expect.stringContaining('claude  claude@example.com  id claude-wsl  active')
    )
  })

  it('lists accounts without forcing a provider usage refresh', async () => {
    // Why: the forced lane bypasses the poll throttle and costs one serial round-trip
    // per managed account. accounts.snapshot is served from the cache and never
    // refreshes, so it is the stronger form of the same guarantee — `--refresh` is the
    // only way to pay for a provider fetch.
    callMock.mockResolvedValue({
      id: 'test',
      ok: true,
      result: {
  ...
  it('forces a provider usage fetch only when --refresh is passed', async () => {
    callMock.mockResolvedValue({
      id: 'test',
      ok: true,
      result: {
