...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Orca Lab host',
    usage: 'orca account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Orca Lab runtime.',
      ...
      '--agent defaults to claude. Requires the Orca Lab runtime to be running on this machine.'
    ],
    examples: ['orca account add', 'orca account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Orca Lab host',
    ...
    notes: [
      ...
      'Its state is `account` (that account), `none` (no managed account; the login in Orca Lab’s shared runtime) or `unknown` with a reason. Unknown is never a licence to fall back to `active`.',
