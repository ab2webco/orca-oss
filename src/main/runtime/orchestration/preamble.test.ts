...
describe('buildDispatchPreamble', () => {
  ...
  it('includes worker_done command with --body 3-sentence summary prompt and reportPath', () => {
    ...
    expect(result).toContain('--envelope-file "<path to the envelope JSON you just wrote>"')
    expect(result).toContain('"status": "success"')
    expect(result).toContain('"level": "live"')
    expect(result).toContain('status "success" cannot carry a "none" claim')
