  ...
  it('drains a terminal retry from the page after the first terminal report', async () => {
    ...
    runtime.setOrchestrationDb({
      ...
      importFederatedRelayItem: ({
        ...
        lifecycle:
          | { kind: 'worker_report'; outcome: 'succeeded' | 'failed'; result: string }
          | { kind: 'none' | 'heartbeat' | 'rejected' }
      }) => {
        if (lifecycle.kind === 'worker_report') {
          expect(JSON.parse(lifecycle.result)).toMatchObject({ envelope: SUCCESS_ENVELOPE })
        }
