...
export function formatMessageBanner(
  ...
  if (authority === 'current') {
    // Why --from at all: older shells can lack Orca's terminal identity environment. A token-bound
    // pane authenticates ahead of this flag, so --from cannot grant an identity it does not hold.
    // Why it is skipped for run:/dispatch: those are addressing scopes, not terminal identities,
    // so there is nothing for --from to name.
    const explicitFrom =
