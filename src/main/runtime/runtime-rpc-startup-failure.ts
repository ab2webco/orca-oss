...
> = {
  permission_denied: {
    ...
    fallback:
      "Orca Lab couldn't write its runtime file. Check permissions on Orca Lab's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart Orca Lab.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "Orca Lab's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart Orca Lab."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart Orca Lab to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart Orca Lab to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'Orca Lab CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "Orca Lab couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'Orca Lab will continue to work, but commands such as orca status, orca terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
