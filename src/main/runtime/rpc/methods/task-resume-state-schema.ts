...
export const TaskResumeState = z
  .object({
    ...
    // Why: Plane is a lab-only provider, so upstream's schema omits it; the parity assertion
    // below is what caught the gap when this file arrived.
    planePreset: z.enum(['everything', 'assigned', 'created', 'all', 'done']).optional(),
    planeQuery: z.string().optional(),
