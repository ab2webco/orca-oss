...
type DaemonAttachmentPrivate = {
  ...
  startupCommandWithheldSessionIds: Set<string>
}
...
describe('DaemonServer attachment lifecycle', () => {
  ...
  it('clears withheld startup state when its session is reaped', async () => {
    await createSession('reaped-session')
    const daemon = server as unknown as DaemonAttachmentPrivate
    await client.request('detach', { sessionId: 'reaped-session' })
    daemon.startupCommandWithheldSessionIds.add('reaped-session')

    await daemon.host.kill('reaped-session')

    expect(daemon.startupCommandWithheldSessionIds.has('reaped-session')).toBe(false)
  })
