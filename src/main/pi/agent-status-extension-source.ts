...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Orca Lab receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Orca Lab hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Orca Lab',
    '    // is unavailable or the loopback request failed (e.g. Orca Lab restart).',
