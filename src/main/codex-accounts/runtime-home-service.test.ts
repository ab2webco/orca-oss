...
describe('CodexRuntimeHomeService', () => {
  ...
  // Why these six were dropped rather than repaired (ORCA-201): they pinned the
  // shared-runtime-mirror lane for MANAGED host accounts through the
  // ORCA_CODEX_SYSTEM_DEFAULT_REAL_HOME env override. Upstream retired both the
  // override and that lane — managed accounts now always run in their own
  // self-contained home, and the injectable setRealHomeLaneGate that replaced the
  // env flag only governs the host SYSTEM-DEFAULT selection. No production path
  // ever set the flag (its own doc said the feature ships unconditionally), so no
  // shipped behaviour lost coverage; upstream's 'routes a host MANAGED account to
  // its own self-contained home' and 'keeps per-account auth canonical when the
  // real-home lane takes over' assert what actually runs now.
