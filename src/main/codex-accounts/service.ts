...
export class CodexAccountService {
  ...
  private assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `Orca Lab cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Orca Lab left your config unchanged.`
    )
  }
  ...
  private assertManagedHomePath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        ...
        throw new Error('Managed WSL Codex home is outside Orca Lab account storage.')
      }
      if (
      ...
      if (!existsSync(join(candidatePath, '.orca-managed-home'))) {
        throw new Error('Managed Codex home is missing Orca Lab ownership marker.')
      }
      if (
        expectedAccountId !== undefined &&
