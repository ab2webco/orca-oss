    ...
    async () => {
      ...
      try {
        ...
        for (const fileName of mainScripts) {
          ...
          // Orca env is missing, so the writer may break — EPIPE, ECONNRESET when Windows tears
          // the pipe down first, or EOF when the handle is already gone by the time libuv
          // reports the failed write (windows-latest, antigravity-hook.cmd). All three say the
          // same thing: the child exited without owning stdin, and the exit code is asserted
          // separately, so widening this cannot hide a hook that failed. hookEnvironment()
          // strips every ORCA_* var, so this relaxation only ever covers the missing-env path —
          // a happy-path case added to this loop must not reuse it.
