  ...
  async getSubmoduleStatus(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        throw new Error(
          'SSH submodule diff support is unavailable on this relay. Reconnect the SSH target to update Orca Lab on the host, then try again.'
        )
      }
      throw error
    }
  ...
  async forceDeletePreservedBranch(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        ...
        throw new Error(
          'This SSH host is running an older Orca Lab relay that cannot delete preserved branches. Reconnect to deploy the latest relay, then try again.'
        )
      }
      throw error
    }
  ...
  async clone(
    ...
    return this.runWithGitReadInvalidation(async () => {
      ...
      try {
        ...
        if (isJsonRpcMethodNotFoundError(error)) {
          throw new Error(
            'SSH clone support is unavailable on this relay. Reconnect the SSH target to update Orca Lab on the host, then try again.'
          )
        }
        throw error
      } finally {
