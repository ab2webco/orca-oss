...
describe('skill discovery', () => {
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    ...
    await writeFile(join(realSkill, 'SKILL.md'), '# Orca Lab CLI\n\nUse the Orca Lab CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'Orca Lab CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    ...
    await writeFile(
      ...
      ['---', 'name: orca-cli', 'description: Use the Orca Lab CLI.', '---', ''].join('\n')
    )
