...
/** A capacity refusal, carrying the code a caller keys off instead of parsing prose.
 *
 *  Why declared here and not in a shared error module: this tree has no browser
 *  error taxonomy, and inventing one to carry a single code would be a bigger
 *  change than the cap itself. Upstream throws its `BrowserError` with the same
 *  code, so a caller that already handles that keeps working.
 */
export class OffscreenBrowserTabCapacityError extends Error {
  readonly code = OFFSCREEN_BROWSER_TAB_CAPACITY_CODE

  constructor(openTabs: number, maxTabs: number) {
    super(offscreenBrowserTabCapacityMessage(openTabs, maxTabs))
    this.name = 'OffscreenBrowserTabCapacityError'
  }
}
