...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not Orca Lab's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Orca Lab restart',
    ...
    '      // writeEndpointFile on the Orca Lab side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Orca Lab restart. The',
    '  // file is rewritten on every Orca Lab start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Orca Lab',
    ...
    '// per turn through Orca Lab (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root Orca Lab pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer Orca Lab since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-Orca Lab-instance.',
    '  const coords = resolveHookCoords();',
    ...
    '    // because Orca Lab is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
    '    clearTimeout(timeout);',
    '  }',
    ...
    '          // publish a final idle so Orca Lab does not retain a dead owner.',
