...
// Why: takes the lab's launch target (selection target + optional per-worktree
// overrideAccountId pin) so pinned launches resolve their own managed home.
...
export function registerPtyHandlers(
  ...
  const bindProviderListeners = (): void => {
    ...
    localExitUnsub = boundProvider.onExit((payload) => {
      // Why: a stale incarnation's exit must not tear down the pane that a newer
      // incarnation of the same session id now owns (upstream #9687).
  ...
  runtime?.setPtyController({
    ...
    spawn: async (args) => {
      ...
      // Why no switch-in-progress throw here: the check runs below, after the per-worktree pin
      // resolution, so injected (pinned) launches stay exempt from the global block.
      ...
      // Why both: isNewDaemonSession gates daemon session-model decisions, while
      // isMintedSessionId also covers the degraded provider's local-fallback ids (ORCA-114),
      // which are minted here and have no daemon session model at all.
      ...
      try {
        ...
        try {
          ...
          if ((isMintedSessionId || isNewDaemonSession) && sessionId !== undefined) {
        ...
        // spawn adopted an existing pane PTY, which launched no CLI here — binding an account
        // to it would hold a reservation for a process Orca never started.
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const isClaudeLaunch =
          !preAdoptedStablePane && !args.connectionId && isClaudeLaunchCommand(args.command)
        // Why no switch-in-progress throw here: the checks run below, after the per-worktree pin
        // resolution, so injected (pinned) launches stay exempt from the global block.
        const terminalRuntimeOptions =
          process.platform === 'win32' && !args.connectionId
            ? resolveLocalWindowsTerminalRuntimeOptions({
        ...
        // Why (ORCA-114): the degraded daemon routes fresh spawns to the local provider, whose
        // numeric counter ids carry no worktree — mint the same shape here so attribution survives
        // the fallback. Host-env setup stays gated on isDaemonHostSpawn; only the id is decoupled.
        // An adopted pane mints nothing: it reuses the owner PTY it just attached to.
        const localFallbackSessionId =
          !preAdoptedStablePane &&
          ...
          runtime?.registerPty(
            ...
            typeof args.tabId === 'string' &&
              ...
              ? {
                  ...
                  leafId: metadataLeafId,
                  ...(result.incarnationId ? { incarnationId: result.incarnationId } : {}),
                  // Why: a reattach passes the launch description without a token — see
                  // resolveRegisteredPtyLaunchBinding. The token itself rides in
                  // agentLaunchAuthority, which refuses reattaches outright.
