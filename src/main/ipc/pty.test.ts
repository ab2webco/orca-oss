...
describe('registerPtyHandlers', () => {
  ...
  it('rejects renderer persistence when a local PTY exits before spawn settles', async () => {
    ...
    const store = { persistPtyBinding: vi.fn(), getWorktreeMeta: vi.fn(() => null) }
    setLocalPtyProvider(provider as never)
    registerPtyHandlers(
      mainWindow as never,
      runtime,
  ...
  describe('spawn environment', () => {
    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      // Note: inherited Claude session-marker stripping moved to the spawn-site
      // module (src/main/pty/claude-session-env.ts, adopt #10188); its coverage
      // lives in claude-session-env.test.ts + the three spawn-site suites.
