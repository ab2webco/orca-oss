...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Orca Lab instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Orca Lab process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Orca Lab instance running for the same profile.'
