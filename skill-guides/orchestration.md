...
- A `sleeping` worker cannot be reached by `terminal send`, `terminal wait`, or a group address; it must be woken from the Orca app first. Its handle answers `terminal show` and refuses writes with `terminal_asleep` — that error means "wake it", not "it died".
