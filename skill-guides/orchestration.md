...
name: orchestration
description: >-
  Use Orca Lab orchestration for structured multi-agent coordination: threaded
  ...
  ordinary terminal control, lightweight terminal prompts, shell commands, Orca Lab
  worktree management, reading or waiting on terminals, and automation of the
  browser embedded inside Orca Lab. Use Computer Use for browser windows, webviews,
  Orca Lab app UI, or desktop UI outside Orca Lab's embedded browser.
---

# Orca Lab Inter-Agent Orchestration

Orchestration is Orca Lab's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.
...
If a task says to use Orca Lab orchestration, the coordinator must create or bind a Run, create the Task with `orca orchestration task-create`, then attach the worker with either the preferred `orca orchestration worker-start` composition or the low-level `orca orchestration dispatch --inject` path.

Do not substitute non-Orca Lab subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create Orca Lab task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.
...
If the work was accidentally run outside Orca Lab orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh Orca Lab terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `orca orchestration` commands are RPC calls to the running Orca Lab runtime.

## Contract Migration

Orca Lab adopts a live pre-update orchestration assignment into an ordinary Run. Adoption preserves the existing agent process, PTY/session, terminal handle, tab/leaf/pane, worktree or folder workspace, Task, and Dispatch; it never restarts or replaces the worker. The retired scheduler is not revived, and a newly created attempt uses the current grammar.
...
When a compatibility or recovery command returns structured next-step arguments, run those exact arguments with the same CLI executable. The arguments intentionally omit the executable name so the guidance works with `orca`, `orca-ide`, `orca-dev`, or another configured Orca Lab CLI command. Do not translate the command from memory, broaden its recipient, or retry it as a current mutation unless the returned guidance explicitly says to.
...
New orchestration messages and tasks belong to one explicitly bound Run. A Run is only a durable namespace and coordinator inbox; it never schedules or places workers. Lifecycle authority comes from the active Dispatch, and terminal handles remain routing metadata rather than durable identity. Send `worker_done` and `heartbeat` from the worker's own terminal; Orca Lab routes them to that Dispatch's Run.
...
- Omit `--from` unless impersonating another terminal; Orca Lab auto-resolves it from the current terminal.
...
- Use `dispatch:<id>` for coordinator guidance to one supervised worker. Orca Lab routes that stable address locally or through the connected-server relay; do not substitute a remote terminal handle.
...
- Treat a `check --wait` timeout or `{count:0}` as a checkpoint, not a worker failure. Long coding tasks routinely run 15-60 minutes; keep using rolling waits unless you receive `worker_done`/`escalation`, `terminal list` reports the worker `gone`, or the user explicitly asks you to stop.
...
- Read a worker's `liveness` from `terminal list`, never its absence or `connected`. `running` is live, `sleeping` is a hibernated agent whose session is intact, `gone` is the only value that means the work stopped. Dispatching a replacement onto a `sleeping` worker's branch puts two agents on one branch.
- A `sleeping` worker cannot be reached by `terminal send`, `terminal wait`, or a group address; it must be woken from the Orca Lab app first. Its handle answers `terminal show` and refuses writes with `terminal_asleep` — that error means "wake it", not "it died".
...
- `heartbeat` is also Dispatch-scoped. Include both IDs and omit `--to` so Orca Lab uses the owning Run; use `status` for broad progress updates.
...
Use `worker-start` for the normal supervised path. It composes the existing worktree, terminal, readiness, and dispatch primitives while returning exact created/reused effects. Agents still choose placement and concurrency; Orca Lab does not schedule workers or infer conflicts.
...
`--effort` requires `--model`, and neither option can combine with `--terminal`. A connected worker server must advertise launch-preference support before Orca Lab forwards either option.
...
To run the worker on another connected Orca Lab server, add `--on <saved-environment>`. The Run and Tasks remain authoritative on the current server; later commands route by Dispatch ID, so never repeat `--on`:
...
`orchestration check` receives it even when the Dispatch is on another connected Orca Lab server.

`worker-read` defaults to `--source auto`: Orca Lab returns the exact hook-reported Codex, Claude, OpenClaude, or Grok transcript when it can prove the worker session, otherwise it returns bounded terminal output with `source: "terminal"` and a typed `fallbackReason`. Continue with the returned top-level `cursor`; it stays pinned to that exact source. If Orca Lab reports `source_changed`, start a fresh read without the old cursor. Never supply or guess a provider session ID or transcript path.
...
After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `orca orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Orca Lab transfers cleanup ownership to the new Dispatch. Otherwise run `orca orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: Orca Lab first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities Orca Lab cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `orca orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
Workers report exactly once using the IDs and capability injected by Orca Lab; they do not supply Run/server/terminal identity:
...
When you place a supervised worker with `terminal create --agent` or `worktree create --agent` instead of `worker-start`, pass `--task <task_id>` so the worker still gets a lifecycle: Orca Lab waits for the agent to become idle, dispatches the task onto that terminal, and injects the same `worker_done` preamble. A supervised agent terminal created without a Task/Dispatch has no completion signal — it finishes and idles silently, leaving the coordinator to poll `terminal read` and guess. Do not pass `--task` for full handoffs; it creates supervised lifecycle obligations.
...
Before creating a new worktree from an active feature branch, decide and state whether the desired Orca Lab lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.
...
`--no-parent` only controls Orca Lab lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Orca Lab uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `orca repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
When a new worktree is allowed, use child lineage for isolated work that is stacked under or dependent on the active worktree, and use `--no-parent` when it is not stacked. Decide the Git base separately: `--no-parent` makes the worktree top-level in Orca Lab, while omitted `--base-branch` uses the repo default base.
...
- A failed outcome is still a terminal report, but Orca Lab records both the Dispatch and Task as failed. Never encode failure only in the subject/body.
