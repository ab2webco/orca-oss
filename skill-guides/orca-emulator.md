...
name: orca-emulator
description: >
  Control a mobile (iOS) emulator / simulator stream from inside Orca Lab using the `orca` CLI.
  Use for taps, gestures, typing, hardware buttons, camera injection, permissions, accessibility tree, and more — all while seeing the live view in Orca Lab's emulator pane.
  Prefer this over raw `npx serve-sim` or direct simctl when running agents inside Orca Lab (the orca surface handles device scoping, helper lifecycle, and worktree context).
...
Drive an Apple Simulator (iOS / iPad / Watch) **from within Orca Lab** using `ORCA emulator ...` commands (or `ORCA emulator exec` for raw power). This wraps the excellent [serve-sim](https://github.com/EvanBacon/serve-sim) open-source tool so agents get a consistent Orca Lab-native CLI surface, automatic helper management, and seamless integration with Orca Lab's live emulator pane (the visual "preview" surface).

The underlying serve-sim helper captures the real simulator framebuffer (via private SimulatorKit / IOSurface for low-latency 60fps H.264 or MJPEG) and exposes a WebSocket control channel. Orca Lab's bridge owns the helper processes and per-worktree "active emulator" state so unqualified commands "just work" on whatever device/pane is current for the worktree.

## CLI executable

Choose the Orca Lab executable once: use the `ORCA_CLI_COMMAND` environment value when set;
otherwise use `orca-dev` in a dev session exposing `ORCA_DEV_REPO_ROOT`, `orca-ide` on
Linux outside an Orca Lab-managed terminal, and `orca` everywhere else. Never try bare
...
- The user/agent wants to **tap, swipe, drag, pinch, or press hardware buttons** on a running iOS simulator while seeing the live result in Orca Lab.
...
- You are inside an Orca Lab worktree/terminal and want the emulator to be **workspace-scoped** (like browser tabs) with explicit targeting when needed.
- The agent should use Orca Lab's preview pane instead of external Simulator.app or raw serve-sim URLs.
...
## Prerequisites (enforced / surfaced by Orca Lab)

- macOS host (with Xcode Command Line Tools: `xcrun --version`).
- A booted simulator (`xcrun simctl list devices booted` or let Orca Lab/attach help boot one).
- Node available (for the serve-sim bits; Orca Lab bundles the CLI surface).
- macOS 14+ recommended for full camera injection features.

Orca Lab will give clear errors if these are missing (e.g. "emulator commands require macOS + Xcode tools").
...
Agents use the Orca Lab executable chosen above (on PATH in Orca Lab terminals) and never have to manage PIDs, state files in /tmp, or raw WS URLs themselves.
...
| Goal                        | Command                                      | Notes |
|-----------------------------|----------------------------------------------|-------|
| List available / running   | `ORCA emulator list [--worktree <sel>]`     | Shows Orca Lab-managed + raw serve-sim streams. Use output for explicit --device/--emulator. |
...
| Stop                       | `ORCA emulator kill [--device <id>]`        | Or let pane close / Orca Lab quit clean up. |

Most support `--worktree <selector>` and explicit `--device <udid|name>` or `--emulator <id>` (from list) for targeting.

## Critical gotchas (teach agents)

- **Prefer `tap` over `gesture` for single taps** (same as raw serve-sim). Separate gesture begin/end can be interpreted as long-press due to WS overhead. The Orca Lab wrapper uses the reliable quick sequence.
...
- Stale helpers / state are cleaned by Orca Lab on quit, but agents should `kill` when done.
- Private APIs under the hood (SimulatorKit etc.) — version sensitive (Xcode updates can affect).

## Targeting devices & worktrees

- Default: current worktree's active emulator (resolved from shell cwd or Orca Lab context).
...
- Orca Lab-generated emulator id (for stability, like browserPageId): use `--emulator <id>` returned by list (recommended for scripts that persist ids).

`--worktree all` only for listing.

## Integration with the live pane (UI)

- Opening the emulator pane in Orca Lab (or `attach`) makes that stream the "active" one for the worktree → CLI commands target it automatically.
...
Or let Orca Lab quit / close the pane.

Orphans are cleaned by Orca Lab (like agent-browser sessions).
...
Confirm `ORCA status --json` and `ORCA emulator list --json`, then drive the emulator while the live view is visible in Orca Lab.

See also: orca-cli skill (terminals, worktrees, built-in browser), computer-use for desktop outside the simulator.

This skill is the Orca Lab-native replacement for raw serve-sim when you want the visual + control integrated in the IDE.
