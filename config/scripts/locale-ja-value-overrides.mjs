...
const JA_AUDIT_OVERRIDES = {
  ...
  'Split Up': '上に分割',
  'Split Down': '下に分割',
  "Install the Browser Use skill so agents can operate Orca Lab's browser.":
    'Browser Use スキルをインストールすると、 Agent が Orca Lab のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from Orca Lab — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。Orca Lab から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch Orca Lab if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は Orca Lab を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this Orca Lab server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート Orca Lab サーバー] → [この Orca Lab サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  ...
  'only when Orca Lab cannot derive the API base URL from the git remote.':
    'は、Orca Lab が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when Orca Lab cannot derive the API URL from the remote.':
    'は、Orca Lab がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, Orca Lab refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、Orca Lab はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. Orca Lab skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、Orca Lab は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from Orca Lab':
    '含まれている {{value1}} {{value0}} 件を Orca Lab から削除',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Orca Lab server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート Orca Lab サーバー] → [この Orca Lab サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving Orca Lab':
    'Orca Lab から離れずに、タスクから Agent を開始',
  ...
  'Rename Orca Lab-created branches from the initial agent task.':
    'Orca Lab が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  'Dim files matched by .gitignore in the file explorer.':
    'ファイルエクスプローラーで、.gitignore に一致するファイルを薄く表示します。',
  'Command line Orca Lab runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに Orca Lab が実行するコマンドライン。',
  ...
  'Repository not in Orca Lab': 'このリポジトリは Orca Lab に登録されていません',
  ...
  'Clears them from Orca Lab only. Remote files, worktrees, and branches are left untouched.':
    'Orca Lab からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the Orca Lab CLI before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、Orca Lab CLI をインストールしてください。',
  'Run grok in a terminal on the computer running Orca Lab and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'Orca Lab を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "Orca Lab works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'Orca Lab はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Install and manage experimental Orca Lab plugins.':
    '実験的機能の Orca Lab プラグインをインストール・管理します。',
  "Resident memory held by Orca Lab plus the processes under each worktree's terminals.":
    'Orca Lab が保持する常駐メモリと、各ワークツリーのターミナル配下で実行中のプロセス。',
  ...
  'Optional. Orca Lab can use your normal Codex login; add accounts only if you want quick switching in Orca Lab.':
    '任意。Orca Lab は通常の Codex ログインを使用できます。Orca Lab 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Orca Lab can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。Orca Lab は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Orca Lab works with your existing provider logins; add accounts only if you want Orca Lab to help switch between them.':
    '任意。Orca Lab は既存のプロバイダーのログインと連携します。Orca Lab に切り替えを任せたい場合にのみ、アカウントを追加してください。',
