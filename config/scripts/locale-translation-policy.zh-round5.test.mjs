...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Orca Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Orca Lab IDE',
        localeValue: 'Orca Lab集成开发环境',
        locale: 'zh'
      })
    ).toBe('Orca Lab IDE')
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Orca Lab first',
        localeValue: 'Orca Lab第一',
        locale: 'zh'
      })
    ).toBe('Orca Lab 优先')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Orca Lab Mobile Button',
        localeValue: '显示 Orca Lab 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Orca Lab Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Orca Lab Attribution',
        localeValue: 'Orca Lab归属',
        locale: 'zh'
      })
    ).toBe('Orca Lab 归因')
    expect(
  ...
  it('keeps brand names English and generic workflow terms translated', () => {
    ...
    expect(
      repairTranslatedValue({
        key: 'auto.components.AiVaultSessionSubagents.subagentsCount',
        enValue: '{{count}} subagents',
        localeValue: '{{count}} 代理',
        locale: 'zh'
      })
    ).toBe('{{count}} 智能体')
    expect(
