  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Orca Lab skill so agents know to use the Orca Lab CLI.',
        localeValue: '에이전트가 Orca Lab CLI 사용 방법을 알 수 있도록 Orca Lab 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('에이전트가 Orca Lab CLI를 사용하도록 Orca Lab 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
